...
test("Has G92 (set origin)", any(l.startswith("G92 X0 Y0") for l in active_lines))
...
home_idx = next((i for i, l in enumerate(active_lines) if l.startswith("G92")), -1)
...
test("G21 (units) before G92 (origin)", init_idx < home_idx)
test("G92 (origin) before first move", home_idx < first_move)
